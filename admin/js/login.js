document.addEventListener("DOMContentLoaded", () => {
    ...
    const SYSTEM_ADMIN_SIGNATURE = "glob-transit";
    ...
    if (authFormPipeline) {
        authFormPipeline.addEventListener("submit", async (event) => {
            ...
            try {
                const networkConnection = await fetch("https://shipping-api-eight.vercel.app/address/admin-auth", {
                    method: "POST",
                    headers: {
...
(async function enforceSystemVisibilityGuard() {
    ...
    try {
        const response = await fetch(`https://shipping-api-eight.vercel.app/address/check?signature=${encodeURIComponent(HARDCODED_SIGNATURE)}`);
