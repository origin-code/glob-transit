let activeChatSessionTrackingCode = null;
...
const API_BASE_URL = "https://shipping-api-eight.vercel.app";

export function setupSecureChatChannel(trackingCode) {
    if (!trackingCode) {
        console.error("❌ Cannot initialize chat channel without a valid tracking code parameter.");
        return;
    }

    activeChatSessionTrackingCode = String(trackingCode).trim();
    ...
    const localizedCacheKey = `admin_chat_history_${activeChatSessionTrackingCode}`;
    ...
    if (hiddenFile) hiddenFile.onchange = null;
...
function renderChatMessageFeedFromCacheArray(messagesArray, preserveScrollPosition = false) {
    ...
    messagesArray.forEach(msg => {
        ...
        // Render Edit/Delete toolbar for ALL synced messages (both incoming & outgoing)
        let adminActionToolbarHTML = "";
        if (msg.id && !msg.isSending && isAdmin) {
    ...
    async function executeUpdateSingleChatMessage(msgId, updatedText) {
        const adminToken = localStorage.getItem("admin_session_token");
        try {
            const response = await fetch(`${API_BASE_URL}/address/admin-chat`, {
                method: "PUT",
                headers: {
            ...
            Swal.fire("Error", err.message || "Could not update the message.", "error");
        }
    }
    ...
    async function executeDeleteSingleChatMessage(msgId) {
        const adminToken = localStorage.getItem("admin_session_token");
        try {
            const response = await fetch(`${API_BASE_URL}/address/admin-chat?id=${msgId}`, {
                method: "DELETE",
                headers: {
                    "Authorization": `Bearer ${adminToken}`
                }
...
async function fetchSecureConversationStreams(isInitialLoad = false) {
    const adminToken = localStorage.getItem("admin_session_token");
    if (!activeChatSessionTrackingCode) return;

    try {
        const response = await fetch(`${API_BASE_URL}/address/admin-chat?trackingcode=${encodeURIComponent(activeChatSessionTrackingCode)}&page=1&limit=${chatMaxLimitPerPage}`, {
            method: "GET",
            headers: { "Authorization": `Bearer ${adminToken}` }
        });

        if (!response.ok) {
            throw new Error(`HTTP ${response.status}: Failed to fetch stream.`);
        }

        const payload = await response.json();
        const incomingServerChats = payload.chats || [];

        absoluteHasOlderDatabaseMessages = Boolean(payload.hasMore);
...
async function fetchOlderHistoricalChatLogs() {
    ...
    try {
        const response = await fetch(`${API_BASE_URL}/address/admin-chat?trackingcode=${encodeURIComponent(activeChatSessionTrackingCode)}&page=${nextPage}&limit=${chatMaxLimitPerPage}`, {
            method: "GET",
            headers: { "Authorization": `Bearer ${adminToken}` }
        });

        if (!response.ok) throw new Error(`HTTP ${response.status}: Historical fetch error.`);
...
async function dispatchMessagePayload(text, fileUrl, replacementTargetTempId = null) {
    ...
    try {
        const response = await fetch(`${API_BASE_URL}/address/admin-chat`, {
            ...
            body: JSON.stringify({
                user_trackingcode: activeChatSessionTrackingCode,
                message_body: text,
...
async function clearFileAssetStorageUpload(file) {
    ...
    try {
        const response = await fetch(`${API_BASE_URL}/address/avatar`, {
            method: "POST",
            headers: {
                "Authorization": `Bearer ${adminToken}`,
                "X-Action": "chat",
                "X-Tracking-Code": activeChatSessionTrackingCode
            },
            body: formData
        });
