// Global variable to track live subscription state from database
let isServerSubscribed = false;
const signature = "glob-transit";

document.addEventListener("DOMContentLoaded", async () => {
    await fetchAdminSubscriptionStatus();
    initStepNavigation();
...
/**
 * Fetch live subscription status directly from the database via backend endpoint
 */
async function fetchAdminSubscriptionStatus() {
    const adminToken = localStorage.getItem("admin_session_token");


    try {
        const response = await fetch("https://shipping-api-eight.vercel.app/address/admin-subscription", {
            method: "GET",
            headers: {
                "Authorization": `Bearer ${adminToken}`,
                "x-setting-target": signature
            }
        });

        const result = await response.json();
        if (response.ok && result.success) {
            isServerSubscribed = Boolean(result.admin_subscription);
        }
    } catch (err) {
        console.warn("⚠️ Failed to verify live subscription from DB, defaulting to false:", err);
        isServerSubscribed = false;
    }
}

/**
 * Helper to check active subscription state fetched from DB
 */
function checkIsAdminSubscribed() {
    return isServerSubscribed;
}
...
function initShipmentFormSubmission() {
    ...
    form.onsubmit = async (e) => {
        ...
        try {
            const response = await fetch("https://shipping-api-eight.vercel.app/address/add-shipment", {
                method: "POST",
                headers: {
                    "Authorization": `Bearer ${adminToken}`,
                    "x-setting-target": signature
...
async function executeAvatarNetworkAction(account, fileObject, streamActionType) {
    ...
    const adminToken = localStorage.getItem("admin_session_token");
    const targetUrl = "https://shipping-api-eight.vercel.app/address/avatar";
