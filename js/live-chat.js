document.addEventListener('DOMContentLoaded', () => {
    ...
    function getLocalCachedTrackingCode() {
        let code = localStorage.getItem("user_tracking_code");
        if (!code) {
            try {
                const session = JSON.parse(localStorage.getItem("user_session") || "{}");
                code = session.trackingCode || session.trackingcode || null;
            } catch (e) { }
        }
        return code || null;
    }
    ...
    function clearUserSession() {
        ...
        localStorage.removeItem('user_tracking_code');
    ...
    async function loginWithTrackingCode(trackingCodeInput, submitBtn) {
        const originalBtnText = submitBtn.innerHTML;
        submitBtn.disabled = true;
        submitBtn.innerHTML = `<span>Verifying...</span>`;

        try {
            const res = await fetch(`${API_BASE}/public-shipment?signature=${encodeURIComponent(HARDCODED_SIGNATURE)}&trackingcode=${encodeURIComponent(trackingCodeInput)}`, {
                method: 'GET'
            });

            if (!res.ok) {
                const errData = await res.json().catch(() => ({}));
                throw new Error(errData.error || 'Invalid tracking code provided.');
            }

            const data = await res.json();
            const shipment = data.shipment;
            const token = data.token; // Real JWT returned from backend

            if (!shipment || !token) {
                throw new Error('Tracking record or token not found.');
            }

            const verifiedCode = shipment.trackingcode || shipment.trackingCode || trackingCodeInput;

            const sessionPayload = {
                trackingCode: verifiedCode,
                fullname: shipment.fullname || shipment.receiver_name || "Customer",
            ...
            localStorage.setItem('user_tracking_code', verifiedCode);

            activeTrackingCode = verifiedCode;
            ...
            alert(err.message || 'Unable to authenticate tracking code.');
    ...
    async function initiateChatSession() {
        const token = getUserToken();
        activeTrackingCode = getLocalCachedTrackingCode();

        if (!token || !activeTrackingCode) {
            renderInitialView();
            return;
        }
    ...
    async function fetchConversationFromDB(isInitialLoad = false) {
        const token = getUserToken();
        if (!activeTrackingCode) activeTrackingCode = getLocalCachedTrackingCode();

        if (!activeTrackingCode || !token) return;

        try {
            const res = await fetch(`${API_BASE}/admin-chat?trackingcode=${encodeURIComponent(activeTrackingCode)}&page=1&limit=${CHAT_LIMIT}`, {
                method: "GET",
                headers: {
    ...
    async function fetchOlderHistoricalLogs() {
        ...
        try {
            const res = await fetch(`${API_BASE}/admin-chat?trackingcode=${encodeURIComponent(activeTrackingCode)}&page=${nextPage}&limit=${CHAT_LIMIT}`, {
                method: "GET",
                headers: {
                    "Authorization": `Bearer ${token}`,
                    "X-Setting-Target": HARDCODED_SIGNATURE
    ...
    async function dispatchMessagePayload(text, fileUrl, tempMsgId = null) {
        ...
        try {
            const res = await fetch(`${API_BASE}/admin-chat`, {
                ...
                body: JSON.stringify({
                    user_trackingcode: activeTrackingCode,
                    message_body: text,
    ...
    async function uploadAttachment(file) {
        const token = getUserToken();
        if (!activeTrackingCode) activeTrackingCode = getLocalCachedTrackingCode();
        if (!token || !activeTrackingCode) return null;
        ...
        try {
            const res = await fetch(`${API_BASE}/avatar`, {
                ...
                headers: {
                    "Authorization": `Bearer ${token}`,
                    "X-Action": "chat",
                    "X-Tracking-Code": activeTrackingCode,
                    "X-Setting-Target": HARDCODED_SIGNATURE
                },
                body: formData
            });
    ...
    function renderChatMessageFeed(messagesArray, preserveScroll = false) {
        ...
        elements.feed.innerHTML = `
            <div style="text-align:center; padding:10px; font-size:12px; color:#888;">
                <span>🔒 Support Channel Linked to Tracking ID: ${escapeHTML(activeTrackingCode || '')}</span>
            </div>`;
    ...
    function toggleChatDrawer(show) {
        ...
        if (isChatOpen) {
            ...
            document.body.classList.add('lc-chat-open');
            ...
            document.body.classList.remove('lc-chat-open');
        }
    }
    ...
    function renderInitialView() {
        ...
        if (!token) {
            ...
            formContainer.innerHTML = `
                ...
                <p style="font-size: 13px; color: #666; margin-bottom: 15px;">Enter your tracking code to start chatting with support.</p>
                <input type="text" id="user-tracking-input" placeholder="Tracking Code (e.g. TRK12345)" style="width: 100%; padding: 8px; margin-bottom: 15px; border: 1px solid #ccc; border-radius: 4px; box-sizing: border-box;">
                <button id="user-tracking-login-btn" style="width: 100%; padding: 10px; background: #1f263f; color: #fff; border: none; border-radius: 4px; cursor: pointer; display: flex; align-items: center; justify-content: center; gap: 8px;">
            ...
            const loginBtn = document.getElementById('user-tracking-login-btn');
            loginBtn.addEventListener('click', () => {
                const trackingCodeInput = document.getElementById('user-tracking-input').value.trim();
                ...
                loginWithTrackingCode(trackingCodeInput, loginBtn);
            });
